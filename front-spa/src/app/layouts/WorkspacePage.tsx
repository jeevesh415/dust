...
function useIsPaywallWhitelisted(): boolean {
  const matches = useMatches();
  return matches.some(
    (match) =>
      (match.handle as { paywallWhitelisted?: boolean } | undefined)
        ?.paywallWhitelisted === true
  );
}
...
export function WorkspacePage({ children }: WorkspacePageProps) {
  ...
  const isPaywallWhitelisted = useIsPaywallWhitelisted();
  ...
  // and the current route is not paywall-whitelisted (via route handle).
  // Mirrors the Next.js session.ts logic: redirect to /trial if eligible, /subscribe otherwise.
  if (
    !authContext.subscription.plan.limits.canUseProduct &&
    !isPaywallWhitelisted
  ) {
